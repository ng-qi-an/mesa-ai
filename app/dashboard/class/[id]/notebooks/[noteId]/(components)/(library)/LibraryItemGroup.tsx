...
export default function LibraryItemGroup({label, items, onLabelClick, limit}: {label: string, items: LibraryItemType[], onLabelClick?: (key: string) => void, limit?: number}){
    return <div className="flex flex-col gap-1">
        ...
        {(limit ? items.toSpliced(limit) : items).map((item, index) => <LibraryItem key={index} item={item}/>)}
