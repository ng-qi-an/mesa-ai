...
export function TabItem({label, id, icon, group, ref, closable, isDragging}: {label: string, id: string, icon?: LucideIcon, group: string, ref?: (element: Element | null) => void, closable?: boolean, isDragging?: boolean}) {
  ...
  const Icon = icon
  useEffect(()=>{
    console.log("TabItem rendered", label, id, group, activeTab, isDragging, icon);
  }, [])
  return (
    <button
      ref={ref}
    ...
    >
      {Icon && <Icon size={15} className="mr-3 shrink-0"/>}
