...
export default async function DriveNestPage({params}: {params: Promise<{nestIds: string[] | undefined, id: string}>}) {
    ...
    const nests = nestIds ? await db.select().from(files).where(inArray(files.id, nestIds)) : []
    ...
    return <FileBrowser nests={nests || []} files={userFiles.map(f => ({...f, key: f.id}))}/>
}