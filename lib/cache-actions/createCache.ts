...
export default async function createCache(files: string[], ttl: number = 900){
    ...
    const filesMap = files.map(async(key) => {
        ...
        const cacheFile = await ai.files.upload({
            file: blob,
            config: { name: `user-files/${session.user.id}/${key}`, displayName: key, mimeType: file.ContentType },
        });
        if (!cacheFile.uri || !cacheFile.mimeType) {
